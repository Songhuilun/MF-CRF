...
from networks.MFCRFDepth import MFCRFDepth


parser = argparse.ArgumentParser(description='MFCRF PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode',                      type=str,   help='train or test', default='train')
parser.add_argument('--model_name',                type=str,   help='model name', default='mfcrf')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = MFCRFDepth(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain)
    model.train()
...
def main():
    ...
    if save_files:
        ...
        command = 'cp mfcrf/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp mfcrf/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp mfcrf/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
